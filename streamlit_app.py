...
from enums import HeartRateZone, PaceZone, ExerciseName
from resources.resources import TIMER_HTML, PACE_FORMATTER, SHOE_FORMATTER, STRAVA_EMBED_HTML
...
with st.sidebar:
    ...
    st.write('<a href="#peak-heart-rate-and-pace">Peak Heart Rate and Pace</a>', unsafe_allow_html=True)
...
with st.expander("Strava Activity"):
    html(STRAVA_EMBED_HTML, height=700, scrolling=True)
...
run_duration_distance_chart = {
    "legend": {
        "data": [f"Distance ({'mi' if imperial else 'km'})", "Duration (min)"]
    },
    "xAxis": {
        "type": "category",
        "axisTick": {
        ...
        "data": [
            f"Wk {i + 1}"
            for i, _
            in enumerate(reader.get_tcx("week_start_dates"))
        ] if weekly else reader.get_tcx("dates_str"),
    },
    "yAxis": [
        {
            "type": "value",
            ...
            "alignTicks": True,
            "axisLabel": {
                "formatter": "{value}" + f"{'mi' if imperial else 'km'}"
            }
        },
        {
            "type": "value",
    ...
    "series": [
        {
            ...
            "data": [
                ...
                for d
                in reader.get_tcx('run_distances_weekly' if weekly else 'run_distances_daily')
            ],
            "type": "line",
            "yAxisIndex": 0,
        },
        {
            ...
            "data": [
                round(d / 60, 2)
                for d
                in reader.get_tcx('run_duration_weekly' if weekly else 'run_duration_daily')
            ],
            "type": "line",
            "yAxisIndex": 1,
        },
    ...
    "series": [
        {
            ...
            "stack": 'total',
            "itemStyle": {
                "color": get_color(i + 1, 0, len(HeartRateZone), "Reds")
            },
            "data": data
    ...
    "series": [
        {
            ...
            "stack": 'total',
            "itemStyle": {
                "color": get_color(i + 1, 0, len(PaceZone), "Blues")
            },
            "data": data
...
def make_peak_chart(dataset_name: str, pretty_name: str):
    ...
    if dataset_name == "peak_pace":
        ...
        categories = [name for val, name in DISTANCE_NAMES.items() if val in data.keys()]
        inverse = True
        base_color = "Blues"
        ...
        "series": [
            {
                ...
                "type": "line",
                "itemStyle": {
                    "color": get_color(4, 0, 5, base_color)
                }
            }
        ],
        "tooltip": {
...
with peak_pace_col2:
    st_echarts(options=make_peak_chart("peak_pace", "Pace (min/km)" if not imperial else "Pace (min/mi)"))


def make_peak_pace_chart_by_month():
    data = reader.get_tcx("peak_pace_monthly")

    raw_categories = list(set([int(item) for sublist in data.values() for item in sublist]))
        ...
        "series": [
            {
                "name": months[i],
                "data": [
                    round(ms_to_min_km_or_min_mi(d, imperial), 2)
                    for d in y_values
                ],
                "type": "line",
                "itemStyle": {
                    "color": get_color(i + 1, 0, len(data.keys()), "Blues")
                }
...
grouped_peak_pace_col1, grouped_peak_pace_col2 = st.columns(2, vertical_alignment="center")
...
strength_duration_volume_chart = {
    "legend": {
        "data": [f"Volume ({'lbs' if imperial else 'kg'})", "Duration (min)"]
    },
    "xAxis": {
        "type": "category",
        "axisTick": {
        ...
        "data": [
            f"Wk {i + 1}"
            for i, _
            in enumerate(reader.get_hevy("week_start_dates"))
        ] if weekly else reader.get_hevy("dates_str"),
    },
    "yAxis": [
        {
            "type": "value",
            ...
            "alignTicks": True,
            "axisLabel": {
                "formatter": "{value}" + f"{'lbs' if imperial else 'kg'}"
            }
        },
        {
            "type": "value",
    ...
    "series": [
        {
            ...
            "data": [
                ...
                for d
                in reader.get_hevy("workout_volume_weekly" if weekly else "workout_volume_daily")
            ],
            "type": "line",
            "yAxisIndex": 0,
        },
        {
            ...
            "data": [
                round(d / 60, 2)
                for d
                in reader.get_hevy("workout_duration_weekly" if weekly else "workout_duration_daily")
            ],
            "type": "line",
            "yAxisIndex": 1,
        },
...
one_rep_max_chart = {
    "legend": {
        "data": [v.value for v in ExerciseName]
    },
    "xAxis": {
        "type": "category",
        "axisTick": {
    ...
    "series": [
        {
            "name": exercise,
            "data": [
                round(d if imperial else lbs_to_kg(d), 2) for d in data
            ],
            "type": "line",
